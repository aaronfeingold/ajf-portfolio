...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    ...
    if (!contextLoading && !resumeData.portfolio.projects.find(p => p.id === String(projectId))) {
  ...
  if (loading || contextLoading) {
    return (
      <div className="project-detail">
        <div className="loading-content" style={{ paddingTop: '120px', textAlign: 'center' }}>
          <img
  ...
  if (!project) {
    return (
      <div className="project-detail">
        <div className="row" style={{ paddingTop: '120px', textAlign: 'center' }}>
          <div className="twelve columns">
            <h1 style={{ color: '#fff' }}>Project Not Found</h1>
            <p style={{ color: '#7a7a7a', marginBottom: '30px' }}>
              The project you're looking for doesn't exist or has been removed.
            </p>
            <button onClick={handleBackToPortfolio} className="btn-secondary">
              Back to Projects
            </button>
          </div>
        </div>
